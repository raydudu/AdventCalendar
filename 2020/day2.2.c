...
static const struct passdb_t passdb[] = {
{ 2,5, 'l', "fllxf" },
{ 4,5, 'r', "rrrjmrrrrrrh" },
{ 1,4, 'k', "kkksk" },
...
#define passdbnr (sizeof(passdb)/sizeof(passdb[0]))

int main(int argc, char *argv[]) {
    int goodcnt = 0;

    for (int i = 0; i < passdbnr; i++) {
        if (strlen(passdb[i].pass) < passdb[i].p2) {
            continue;
        }
        if ((passdb[i].pass[passdb[i].p1 - 1] == passdb[i].c &&
                passdb[i].pass[passdb[i].p2 - 1] != passdb[i].c) ||
            (passdb[i].pass[passdb[i].p1 - 1] != passdb[i].c &&
                   passdb[i].pass[passdb[i].p2 - 1] == passdb[i].c)) {
            goodcnt++;
        }
    }

    printf("Passwords checked: %lu -- Good pass count: %d\n", passdbnr, goodcnt);
}
